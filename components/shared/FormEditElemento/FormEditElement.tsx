...
export default function FormEditElement({ dataElement }: FormEditElementProps) {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      await axios.patch(`/api/articulos/${dataElement.id}`, values)
      toast({ title: 'Elemento editado correctamente 👌' })
      rutas.push('/')
    } catch (error) {
      console.log(error)
      toast({
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input {...field} type='text' placeholder='Ingresa el nombre' />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
